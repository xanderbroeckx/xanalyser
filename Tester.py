...
#account_numbers = account_numbers_full[['Unnamed: 0','Codes','Boekjaar', 'Vorig boekjaar']]
#account_numbers['Boekjaar'] = pd.to_numeric(account_numbers.Boekjaar.fillna(0).astype(str).str.replace(".", "", regex=False).str.replace(",", ".", regex=False).replace("XXXXXXXXXX", "0"), errors='coerce').fillna(0)
...
def cashflows(financials, year):
    
    cashflows = {
        'Operational CF': financials['9904'].get('CFY',0) + financials['630'].get('CFY',0) + financials['760'].get('CFY',0) + financials['761'].get('CFY',0) + \
            financials['660'].get('CFY',0) + financials['661'].get('CFY',0) + financials['631/4'].get('CFY',0) + financials['651'].get('CFY',0) + \
            financials['635/8'].get('CFY',0) + financials['6560'].get('CFY',0) + financials['6561'].get('CFY',0) + financials['762'].get('CFY',0) + \
            financials['662'].get('CFY',0) + financials['780'].get('CFY',0) + financials['680'].get('CFY',0) + financials['661'].get('CFY',0) + \
            financials['9125'].get('CFY',0) + 
        }
...
def main():
    ...
    kfi = format_dataframe(KFI(financials, ['CFY','LFY'], option = 'Full'));
    pprint(kfi, depth = 1);
    
    print("\n Ratios: \n");
    ratio = format_dataframe(ratios(financials,['CFY','LFY'], option = 'Full'));
